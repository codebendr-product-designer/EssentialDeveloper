...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    private func makeSUT(url: URL = URL(string: "https://a-url.com")!, file: StaticString = #filePath, line: UInt = #line) -> (
        sut: RemoteFeedLoader, client: HTTPClientSpy) {
            ...
            trackMemoryLeak(sut, client, file: file, line: line)
            return (sut, client)
        }
    
    private func trackMemoryLeak(_ instances: AnyObject..., file: StaticString = #filePath, line: UInt = #line) {
    ...
    private func failure(_ error: RemoteFeedLoader.Error) -> RemoteFeedLoader.Result { .failure(error) }
    ...
    private func expect(_ sut: RemoteFeedLoader, toCompleteWith expectedResult: RemoteFeedLoader.Result, when action: () -> (), file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load { recievedResults in
            ...
            default:
                XCTFail("Expected result \(expectedResult) got \(recievedResults) instead", file: file, line: line)
            }
